...
# 🛠️ Member 2, Option A: The Hybrid Word Bank Manager (File I/O & JDBC)
...
`src/hangman/logic/HybridWordProvider.java`

## Responsibilities

Manage the game's vocabulary pool using a hybrid engine. Read baseline team words from a text file, retrieve personal saved words from a database using JDBC based on the active username, and insert newly created permanent words into the database.
...
Accept the current player's username string passed down from the game coordinator (Member 5). Initialize the `java.io.File` framework to verify `words.txt` exists.

### File Stream Logic (Reading Master List)

Use a `BufferedReader` over a `FileReader` stream to read the shared `words.txt` file sequentially. Extract lines matching the active target difficulty using a string-splitting convention (e.g., `WORD:DIFFICULTY`).

### JDBC Retrieval Logic (Reading Custom List)

Establish a database connection to open a `PreparedStatement`. Run a query filtering custom words where the difficulty matches and the owner username matches the logged-in player (e.g., `SELECT word FROM custom_words WHERE difficulty = ? AND username = ?`). Combine both file-based and database-based matching words into a unified runtime collection and choose a random option.

### JDBC Persistence Logic (Saving Custom Words)

Implement the interface's `addWord(String word, String difficulty)` method. Run an `INSERT` SQL statement through a `PreparedStatement` to push the custom word, its difficulty, and the active player's username into the database table, safely preserving it permanently without touching the team-managed text file.
